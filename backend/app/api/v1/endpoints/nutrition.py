...
from app.ml.nutrition_engine import calculate_nutrition_requirements, score_food_product
...
async def score_food(
    ...
    requirements = calculate_nutrition_requirements(
        ...
        activity_level=pet.activity_level.value, is_neutered=pet.is_neutered,
    )
    product_dict = {
        "calories_kcal": product.calories_kcal, "protein_g": product.protein_g,
        "fat_g": product.fat_g, "allergens": product.allergens or "",
    }
    allergies = pet.known_allergies.split(",") if pet.known_allergies else []
    score_result = score_food_product(
        product=product_dict, requirements=requirements,
        daily_portion_g=payload.daily_portion_g, allergies=allergies,
    )
    return FoodScoringResponse(product_id=payload.product_id, pet_id=payload.pet_id, **score_result)
...
async def recommend_foods(
    ...
    for product in products:
        ...
        score = score_food_product(
            product=product_dict, requirements=requirements,
            daily_portion_g=daily_portion_g, allergies=allergies,
        )
        recommendations.append(FoodRecommendationResponse(
            id=product.id, name=product.name, brand=product.brand,
            species=product.species.value, calories_kcal=product.calories_kcal,
            protein_g=product.protein_g, fat_g=product.fat_g, fiber_g=product.fiber_g,
            allergens=product.allergens, ingredients=product.ingredients,
            is_verified=product.is_verified, overall_score=score["overall_score"],
...
async def estimate_subscription(
    ...
    # Calculate daily amount from calories
    requirements = calculate_nutrition_requirements(
        weight_kg=pet.weight_kg, species=pet.species.value,
        activity_level=pet.activity_level.value, is_neutered=pet.is_neutered,
    )
    # Grams needed = (daily_kcal / product_kcal_per_100g) * 100
    daily_amount_g = round((requirements.daily_calories_kcal / product.calories_kcal) * 100, 1)
    ...
    return SubscriptionEstimate(
        ...
        daily_cost_tl=round((daily_amount_g / 1000) * price, 2),
        weekly_cost_tl=round((weekly_amount_g / 1000) * price, 2),
        monthly_cost_tl=round((monthly_amount_g / 1000) * price, 2),
        price_per_kg=price,
    )
...
async def get_nutrition_plans(
    ...
    stmt = select(NutritionPlan).where(NutritionPlan.pet_id == pet_id).order_by(NutritionPlan.created_at.desc())
    result = await db.execute(stmt)
